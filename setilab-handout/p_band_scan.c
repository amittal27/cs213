...
void* worker(void* arg){
  ...
  int worker_idx = worker_data->worker_idx;
...
int analyze_signal(signal* sig, int filter_order, int num_bands, double* lb, double* ub, 
  ...
  for (int thread_idx = 0; thread_idx < num_threads; thread_idx++) {    
    ...
    worker_input[thread_idx].tid = (pthread_t*) malloc(sizeof(pthread_t));
    if (!worker_input[thread_idx].tid) {
    ...
    worker_input[thread_idx].filter_coeffs = filter_coeffs;
    ...
    int code = pthread_create(worker_input[thread_idx].tid, 
                          NULL,
                          worker,
                          worker_pointer);
    if (code != 0){
  ...
  for (int i = 0; i < num_threads; i++) {
    pthread_t curr_tid = *worker_input[i].tid;
